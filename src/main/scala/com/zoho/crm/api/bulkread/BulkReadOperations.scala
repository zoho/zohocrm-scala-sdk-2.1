...
class BulkReadOperations	{
	def getBulkReadJobDetails( jobId: Long) :Option[APIResponse[ResponseHandler]]	={
	...
	def downloadResult( jobId: Long) :Option[APIResponse[ResponseHandler]]	={
		var handlerInstance :CommonAPIHandler = new CommonAPIHandler()
		var apiPath :String = new String()
		apiPath = apiPath.concat("/crm/bulk/v2.1/read/")
		apiPath = apiPath.concat(jobId.toString())
	...
	def createBulkReadJob( request: RequestWrapper) :Option[APIResponse[ActionHandler]]	={
		var handlerInstance :CommonAPIHandler = new CommonAPIHandler()
		var apiPath :String = new String()
		apiPath = apiPath.concat("/crm/bulk/v2.1/read")
		handlerInstance.setAPIPath(apiPath)
